...
namespace Beamable.Editor.Realms
{
   ...
   public class RealmsService
   {
      ...
      public Promise<List<RealmView>> GetGames()
      {
         ...
         return _requester.Request<GetGameResponseDTO>(Method.GET, $"/basic/realms/games", useCache: true).Map(resp =>
         {
            return resp.projects.Select(p => new RealmView()
            {
               Cid = _api.Cid,
               Pid = p.pid,
               Parent = null,
      ...
      public Promise<List<RealmView>> GetRealms()
      {
         if (string.IsNullOrEmpty(_api.Pid))
         {
         ...
         return _requester.Request<GetGameResponseDTO>(Method.GET, $"/basic/realms/game?rootPID={_api.Pid}", useCache:true)
