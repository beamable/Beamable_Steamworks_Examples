...
namespace Beamable.Editor
{
   ...
   public class EditorAPI
   {
      ...
      public Promise<Unit> SetGame(RealmView game)
      {
         ...
         SaveConfig(CustomerView.Alias, game.Pid, BeamableEnvironment.ApiUrl, CustomerView.Cid);
         return SwitchRealm(game);
      }
      ...
      public Promise<Unit> SwitchRealm(RealmView realm)
      {
         if (realm == null)
         {
            return Promise<Unit>.Failed(new Exception("Cannot switch to null realm"));
         }

         SaveConfig(CidOrAlias, realm.Pid, cid:realm.Cid);
         var requiredUpdates = new List<Promise<Unit>>
         {
            ContentIO.FetchManifest().ToUnit()
         };

         return Promise.Sequence(requiredUpdates).Then(_ =>
         {
            Realm = realm;
            ProductionRealm = realm.FindRoot();
            OnRealmChange?.Invoke(realm);
         }).ToUnit();
      }

      public void SaveConfig(string alias, string pid, string host = null, string cid=null)
      {
